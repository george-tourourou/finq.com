...
/**
 * @file
 * biotic-links.func.php
 */

/**
 * Implements theme_biotic_links().
 */
function theme_biotic_links($variables) {
  ...
  if (count($links) > 0) {
    ...
    foreach ($links as $key => $link) {
      ...
      if (count($children) > 0) {
        ...
        $output .= theme('biotic_links', array('links' => $children, 'attributes' => $attributes));
      }
      $output .= "</li>\n";
    }
    $output .= '</ul>';
