...
Drupal.jsAC = function ($input, db) {
  ...
  $input
    ...
    .focus(function (event) { ac.onfocus(this, event); })
    .blur(function () {
        //ac.hidePopup();
        //ac.db.cancel();
  ...
  for (key in matches) {
    $('<li></li>')
      //.html($('<div></div>').html(matches[key]))
		.html($('<a href="/' + lang + '/instruments/' + key + '?from_search=1">' + matches[key] + '</a>'))//.click(function (e) { e.preventDefault(); }))
